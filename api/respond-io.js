...
// Hard gate: proxy must not send customer replies directly. Replies must go through MCP send_message with explicit contactId.
const PROXY_OUTBOUND_MODE = 'disabled'; // forced
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const mediaBlock = mediaPayload
      ...
      : '';
    const enrichedMessage = `${transformed.message}${contextBlock}${continuationHint}${mediaBlock}`;
